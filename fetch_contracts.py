...
ENV: EIA_KEY (opcjonalne - dla rolling M1-M4)
...
YEAR_START = CURRENT_YEAR - 25
YEAR_END   = CURRENT_YEAR + 3

EIA_KEY = os.environ.get("EIA_KEY", "").strip()
if not EIA_KEY:
    print("[WARN] EIA_KEY nie ustawione - rolling M1-M4 beda pominiete", flush=True)
...
PRODUCTS_C = [
    ...
    dict(pid="HO", name="Heating Oil (NYMEX)", unit="$/gal",
         ...
         eia_series={"M1":"PET.EER_EPD2F_PE1_Y35NY_DPG.D","M2":"PET.EER_EPD2F_PE2_Y35NY_DPG.D",
                     "M3":"PET.EER_EPD2F_PE3_Y35NY_DPG.D","M4":"PET.EER_EPD2F_PE4_Y35NY_DPG.D"},
    ...
    dict(pid="RB", name="RBOB Gasoline (NYMEX)", unit="$/gal",
         ...
         eia_series={"M1":"PET.EER_EPMRU_PE1_Y35NY_DPG.D","M2":"PET.EER_EPMRU_PE2_Y35NY_DPG.D",
                     "M3":"PET.EER_EPMRU_PE3_Y35NY_DPG.D","M4":"PET.EER_EPMRU_PE4_Y35NY_DPG.D"},
    ...
    dict(pid="BZ", name="Brent Crude (ICE)", unit="$/bbl",
         ...
         eia_series=None, yahoo_root="BZ", yahoo_exchange=".NYM"),
...
def fetch_eia_series(series_id):
    ...
    try:
        ...
        print(f"    [EIA {series_id}] HTTP {e.code}: {e.reason}", flush=True); return []
    except Exception as e:
        print(f"    [EIA {series_id}] EXC: {e}", flush=True); return []
    rows = data.get("response", {}).get("data", []) or []
    obs = []
    for r in rows:
        d = (r.get("period") or "")[:10]; v = r.get("value")
...
def fetch_eia_rolling(product_meta):
    ...
    for key, sid in product_meta["eia_series"].items():
        ...
        if obs:
            out[key] = obs
            print(f"    [EIA] {product_meta['pid']}.{key} ({sid}): {len(obs)} obs, {obs[0]['date']}..{obs[-1]['date']}", flush=True)
        else:
...
def _import_yfinance():
    ...
    try:
        import yfinance; _YF_IMPORTED = True; return True
    except ImportError:
        print("[FATAL] yfinance nie zainstalowany", flush=True); return False

def yahoo_ticker(root, month_code, year, exchange):
    return f"{root.upper()}{month_code}{year % 100:02d}{exchange}"

def fetch_yahoo_contract(ticker, unit_scale=1.0):
...
def fetch_yahoo_all_contracts(product_meta, mode):
    if not product_meta.get("yahoo_root") or not product_meta.get("yahoo_exchange"): return {}
    root = product_meta["yahoo_root"]; exch = product_meta["yahoo_exchange"]
    ...
    for year in range(YEAR_START, YEAR_END + 1):
        for mc in MONTH_CODES:
            ...
            if obs:
                ...
                out[key] = obs; ok += 1
                if ok <= 3 or ok % 10 == 0:
                    print(f"    [YF] {ticker}: {len(obs)} obs, {obs[0]['date']}..{obs[-1]['date']}", flush=True)
            else:
                fail += 1
    print(f"    [YF] {product_meta['pid']}: ok={ok}, fail={fail}", flush=True)
    return out

def merge_history(existing, fresh):
    if not existing and not fresh: return []
    combined = {o["date"]: o["value"] for o in (existing or [])}
    for o in (fresh or []): combined[o["date"]] = o["value"]
...
def save_product(pid, meta, rolling, contracts):
    fp = CONTRACTS_DIR / f"{pid}.json"
    out = {
        "product": pid, "name": meta["name"], "unit": meta["unit"],
        "contract_size": meta["contract_size"], "contract_unit": meta["contract_unit"],
        "category": meta["category"], "color": meta["color"],
        "rolling": rolling or {}, "contracts": contracts or {},
        "sources": {"rolling": "EIA" if rolling else None, "contracts": "Yahoo" if contracts else None},
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }
    with fp.open("w") as f: json.dump(out, f, ensure_ascii=False)

def main():
    mode = sys.argv[1] if len(sys.argv) > 1 else "incremental"
    print(f"[fetch_contracts v2 HYBRID] mode={mode}, produktow={len(PRODUCTS_C)}, years={YEAR_START}..{YEAR_END}", flush=True)
    print(f"[fetch_contracts] EIA_KEY: {'SET' if EIA_KEY else 'MISSING'}", flush=True)
    _import_yfinance()
    total_rolling = 0; total_contracts = 0
    for p in PRODUCTS_C:
        pid = p["pid"]
        ...
        if merged_rolling or merged_contracts:
            ...
            print(f"  [SAVED] {pid}: rolling={len(merged_rolling)} serii, contracts={len(merged_contracts)} kontraktow", flush=True)
        else:
            print(f"  [SKIP] {pid}: brak danych", flush=True)
    manifest = {
        "updated_at": datetime.now(timezone.utc).isoformat(),
        "products": [
